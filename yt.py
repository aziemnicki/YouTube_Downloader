...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.slider_progressbar_frame = customtkinter.CTkFrame(
            self, fg_color="transparent", width=50, height=150
        )
        self.slider_progressbar_frame.grid(
            row=2, column=0, padx=(20, 0), pady=(20, 0), sticky="e"
        )
        ...
        self.slider.grid(
            row=0, column=5, rowspan=5, padx=(10, 40), pady=(10, 10), sticky="ns"
        )
        ...
        if self.button_play:
            ...
            for i in range(len(self.queue)):
                ...
                label = customtkinter.CTkLabel(
                    master=self.scroll, text=f"{self.queue[i]}"
                )
                label.grid(
                    row=i,
                    column=0,
            ...
            for f in range(len(files)):
                if files[f] == self.song_title:
                    ...
                    mix.music.play()
            label = customtkinter.CTkLabel(
                master=self.scroll, text=f"{self.song_title}"
            )
            label.grid(
                row=0,
                column=0,
    ...
    def foldery(self):
        if not self.selected:
            self.selected_path = filedialog.askdirectory(
                initialdir=main_path, title="Wybierz folder"
            )
    ...
    def download(self):
        ...
        if link != "":
            ...
            song = yt.streams.filter(
                file_extension="mp4", resolution=yt.streams.get_highest_resolution()
            ).first()
            print(song)
